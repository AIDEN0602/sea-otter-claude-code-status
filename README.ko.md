...
macOS 13+, Claude Code, Xcode Command Line Tools가 필요합니다.
Ghostty ≥ 1.3은 선택 — 없으면 탭 점프와 탭 순서 라벨만 빠집니다.

아래 블록을 통째로 복사해서 터미널에 붙여넣으세요:

```bash
# 0. (최초 1회) 빌드 도구가 없으면 설치
...
# 2. Claude Code 훅 등록 (기존 설정은 먼저 백업됨)
bash engine/install.sh

# 3. 앱을 빌드해서 /Applications 에 설치
bash scripts/build_app.sh
open "/Applications/NotchOtter.app"
```

끝입니다 — 메뉴바/노치 옆에 수달이 나타납니다.

**첫 실행:** 앱이 자체 서명(self-signed)이라 macOS가 "확인되지 않은
개발자"라고 할 수 있어요. Applications에서 `NotchOtter.app`을 **우클릭 →
열기 → 열기** 한 번만 하면 그다음부턴 정상 실행됩니다.

첫 실행 시 macOS가 알림 권한과 Automation 권한(터미널 탭 포커스용)을
